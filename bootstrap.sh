...
# Args: <management_group_id> <subscription_id> <preflight_enabled:true|false> <grant_self_mg_contributor:true|false>
MG_ID="${1:-}"
SUBSCRIPTION_ID="${2:-}"
PREFLIGHT_ENABLED="${3:-true}"
GRANT_SELF_MG_CONTRIBUTOR="${4:-false}"
...
ASSIGN_STATE_FILE=".mg_contributor_assign_id"

# Preflight (optional)
PREFLIGHT_OK="false"
PREFLIGHT_ERROR=""
GRANTED_MG_CONTRIBUTOR="false"
CLEANUP_CMD=""
LOGIN_MSG_ESCAPED=""
SKIP_FURTHER_CHECKS=""
  ...
  if ! az account show >/dev/null 2>&1; then
    ...
    if [ -z "$OBJECT_ID" ]; then
      ...
      if ! echo "$MG_ROLES" | grep -Eiq 'Owner|User Access Administrator|Contributor'; then
        # Insufficient MG permissions: attempt to grant temporary Contributor if enabled
        if [ "${GRANT_SELF_MG_CONTRIBUTOR}" = "true" ]; then
          ASSIGN_ID=$(az role assignment create --assignee "$OBJECT_ID" --role "Contributor" --scope "/providers/Microsoft.Management/managementGroups/${MG_ID}" --query id -o tsv 2>/dev/null || true)
          if [ -n "$ASSIGN_ID" ]; then
            # Successfully granted temporary role; instruct user to re-auth and rerun
            GRANTED_MG_CONTRIBUTOR="true"
            CLEANUP_CMD="az role assignment delete --ids ${ASSIGN_ID}"
            echo "$ASSIGN_ID" > "$ASSIGN_STATE_FILE" 2>/dev/null || true
            PREFLIGHT_OK="false"
            PREFLIGHT_ERROR="Granted temporary Contributor at MG. Please run 'az account clear && az login --use-device-code' and rerun Terraform. On success, bootstrap will remove the temporary role."
          fi
        fi
        if [ "$GRANTED_MG_CONTRIBUTOR" = "true" ]; then
          ...
          PREFLIGHT_OK="false"; PREFLIGHT_ERROR="Insufficient Management Group permissions. Need Owner/User Access Administrator/Contributor on /providers/Microsoft.Management/managementGroups/${MG_ID}. Enable grant_self_mg_contributor=true or have an MG Owner assign the role."
        fi
      else
...
cat <<EOF
{
  ...
  "granted_mg_contributor": "${GRANTED_MG_CONTRIBUTOR}",
